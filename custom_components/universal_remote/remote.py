"""Remote entities for universal remotes backed by infrared entities."""
...
def remote_unique_id(entry_id: str, remote_id: str) -> str:
    """Return the unique id for a configured universal remote."""
...
def _standalone_universal_remote_entity_name(
    ...
    """Return entity name for a standalone universal remote.

    The standalone Universal Remote integration creates one device per remote, so
...
def cleanup_stale_remote_entities(
    ...
    This cleanup is safe for both the standalone Universal Remote integration and
    integrations such as iTach IP2IR which reuse the shared universal remote
...
def cleanup_stale_universal_remote_devices(
    ...
    *,
    ...
    """Remove stale universal-remote device registry entries.

    This is intended for the standalone Universal Remote integration where each
    universal remote is represented as its own device. Do not use this for
    physical-device integrations which attach universal remote entities to their
...
async def async_setup_universal_remote_entities(
    ...
    *,
    ...
    return the configured universal remote name from ``entity_name_factory`` so
    ...
    for remote_config in configured_remote_definitions(entry):
        ...
        if remote_id in configured_remote_ids:
            _LOGGER.debug("Skipping duplicate universal remote id: %s", remote_id)
            continue
...
async def async_setup_entry(
    ...
    await async_setup_universal_remote_entities(
        ...
        entry,
        async_add_entities,
        device_info_factory=_universal_remote_device_info,
        entity_name_factory=_standalone_universal_remote_entity_name,
...
class InfraredRemoteEntity(RemoteEntity):
    """Universal remote backed by a Home Assistant infrared entity."""
    ...
    @property
    def is_on(self) -> bool:
        """Return whether the universal remote is on."""
        return self._is_on
